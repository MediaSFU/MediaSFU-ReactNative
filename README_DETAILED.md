...
<p align="center">
  ...
  <a href="https://reactnative.dev">
    <img src="https://img.shields.io/badge/React_Native-0.82+-61DAFB?style=flat-square&logo=react&logoColor=white" alt="React Native 0.82+" />
  </a>
...
participants, and media lifecycle while your React Native app renders every
visible control and media surface.
...
export function HeadlessRoom() {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <Button title="Leave" onPress={() => void room.controls.leave()} />
...
Replace the example URL with your reachable MediaSFU Open server. For a cloud
release, create and join rooms through your authenticated backend; do not ship
reusable MediaSFU credentials in the app bundle.

The important interfaces are:

- `sourceParameters`: one stable seed object passed to the room component;
- `updateSourceParameters`: accepts every newly published parameter bag;
- `onMediaChanged`: tells the hook to re-read because media actually changed;
- `room.parameters`: the newest bag—do not keep an older copy;
- `room.controls`, `room.moderation`, `room.session`, and `room.produce`: safe
  action groups for your custom UI.

Read current state through the hook. Do not call `getUpdatedAllParams()` from a
render or polling loop; it publishes. The SDK's pure reads use
`getCurrentParams()` internally.
...
2. Look for the `peerDependencies` section:
   ...
   "peerDependencies": {
     "@react-native-async-storage/async-storage": "^1.23.1",
     "@react-native-clipboard/clipboard": "^1.14.3",
     "@react-native-community/slider": ">=5.2.0 <6.0.0",
     "@react-native-picker/picker": "^2.7.5",
     "@react-navigation/native": "^6.1.18",
     "@react-navigation/native-stack": "^6.11.0",
     "mediasoup-client": "^3.20.0",
     "react": ">=19.1.0 <20.0.0",
     "react-color": "^2.19.3",
     "react-native": ">=0.82.0 <0.87.0",
     "react-native-gesture-handler": ">=2.32.0 <3.0.0",
     "react-native-permissions": "^5.0.2",
     "react-native-picker-select": "^9.0.0",
     "react-native-reanimated": ">=4.5.3 <5.0.0",
     "react-native-safe-area-context": ">=5.7.0 <6.0.0",
     "react-native-screens": ">=4.26.0 <5.0.0",
     "react-native-sound": "^0.11.2",
     "react-native-status-bar-height": "^2.6.0",
     "react-native-vector-icons": "^10.2.0",
     "react-native-video": "^6.7.0",
...
```bash
npm install @react-native-clipboard/clipboard @react-native-async-storage/async-storage react-native-webrtc react-native-safe-area-context react-native-picker-select @react-native-picker/picker react-native-reanimated react-native-gesture-handler react-native-sound
```

### Screen orientation

MediaSFU reads the current window dimensions to adapt its layout. The host application controls screen orientation, so this SDK does not require a separate orientation-locking package. Configure orientation rules in your app when a flow needs portrait or landscape behavior.
...
export interface PreJoinPageOptions {
  ...
  /**
   * Optional credentials for MediaSFU Cloud or a self-hosted server.
   */
  credentials?: Credentials;
