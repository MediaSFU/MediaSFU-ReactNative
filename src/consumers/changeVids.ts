  ...
  try {
    ...
    if (shareScreenStarted || shared) {
      ...
      if (alVideoStreams.length > screenPageLimit) {
        ...
        const temp: (Stream | Participant)[] = [];
        await Promise.all(
          ref_participants.map((participant) => {
            const stream = alVideoStreams.find(
        ...
        const youyou = allVideoStreams.find(
          (obj) => obj.producerId === 'youyou',
        );
        if (!youyou) {
          const youyouyou = allVideoStreams.find(
      ...
      await Promise.all(
        ref_participants.map(async (participant) => {
          ...
          } else if (!stream && participant.name !== member && !participant.muted) {
            non_alVideoStreams.push(participant);
          }
        }),
      );
    ...
    if (eventType === 'conference' && islevel !== '2') {
      ...
      if (host) {
        ...
        if (islevel === '2') {
          ...
          const hostVideo = alVideoStreams.find(
    ...
    if (breakOutRoomStarted && !breakOutRoomEnded) {
      const tempBreakoutRooms = JSON.parse(JSON.stringify(breakoutRooms));
      const host = participants.find((obj) => obj.islevel == '2');
      for (let room of tempBreakoutRooms) {
        try {
          const currentStreams: (Stream | Participant)[] = [];
          ...
          for (const participant of room) {
            ...
            const streams = allStreamsPaged.filter((stream) => {
              if ((Object.prototype.hasOwnProperty.call(stream, 'producerId')
                ...
                || (Object.prototype.hasOwnProperty.call(stream, 'audioID')
                  && stream.audioID != null
                  && stream.audioID !== '')) {
                const producerId = stream.producerId || stream.audioID;
                const matchingParticipant = ref_participants.find(
      ...
      const remainingStreams = allStreamsPaged.filter((stream) => {
        if ((Object.prototype.hasOwnProperty.call(stream, 'producerId')
          && stream.producerId != null
          && stream.producerId !== '')
          || (Object.prototype.hasOwnProperty.call(stream, 'audioID')
      ...
      if (memberInRoom) {
        const memberStream = allStreamsPaged.find((stream) => {
          if (Object.prototype.hasOwnProperty.call(stream, 'producerId')
            && stream.producerId != null
            && stream.producerId !== '') {
            return (
      ...
      const remainingPaginatedStreams: (Stream | Participant)[][] = [];
