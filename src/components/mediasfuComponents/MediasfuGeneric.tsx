...
import { Text, View, Pressable, Platform, Dimensions, StatusBar, StyleSheet } from 'react-native';
...
import { createModernReactNativeOverrides } from '../../components_modern/modernOverrides';
...
import {
  ...
  controlMedia as controlPanelistMedia,
...
/**
 * MediasfuGeneric component provides a flexible foundation for building real-time media collaboration experiences.
 * It supports audio/video conferencing, screen sharing, chat, polls, breakout rooms, and recording capabilities
 * with full WebRTC/Mediasoup integration.
 *
 * ### Key Features
 ...
 * @returns {React.FC<MediasfuGenericOptions>} MediaSFU Generic component
 *
 * @example
 * ```tsx
...
const MediasfuGeneric: React.FC<MediasfuGenericOptions> = ({
  ...
  const uiOverrides = React.useMemo(
    () =>
      useModernUI
        ? createModernReactNativeOverrides(providedUIOverrides)
        : providedUIOverrides,
  ...
  const MainAspect = React.useMemo(
  ...
  const PaginationComponent = React.useMemo(
    () => withOverride(uiOverrides?.pagination, Pagination),
  ...
  const ControlButtonsAlt = React.useMemo(
    () => withOverride(uiOverrides?.controlButtonsAlt, ControlButtonsAltComponent),
    [uiOverrides?.controlButtonsAlt],
  );
  const ControlButtonsTouch = React.useMemo(
    () => withOverride(uiOverrides?.controlButtonsTouch, ControlButtonsComponentTouch),
    [uiOverrides?.controlButtonsTouch],
  );
  const LoadingModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.loadingModal, LoadingModal),
    [uiOverrides?.loadingModal],
  );
  const AlertComponentOverride = React.useMemo(
    () => withOverride(uiOverrides?.alert, AlertComponent),
    [uiOverrides?.alert],
  );
  const MenuModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.menuModal, MenuModal),
    [uiOverrides?.menuModal],
  );
  const RecordingModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.recordingModal, RecordingModal),
    [uiOverrides?.recordingModal],
  );
  const RequestsModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.requestsModal, RequestsModal),
    [uiOverrides?.requestsModal],
  );
  const WaitingRoomModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.waitingRoomModal, WaitingRoomModal),
    [uiOverrides?.waitingRoomModal],
  );
  const DisplaySettingsModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.displaySettingsModal, DisplaySettingsModal),
    [uiOverrides?.displaySettingsModal],
  );
  const EventSettingsModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.eventSettingsModal, EventSettingsModal),
    [uiOverrides?.eventSettingsModal],
  );
  const CoHostModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.coHostModal, CoHostModal),
    [uiOverrides?.coHostModal],
  );
  const ParticipantsModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.participantsModal, ParticipantsModal),
    [uiOverrides?.participantsModal],
  );
  const MessagesModalComponent = React.useMemo(
  ...
  const TranslationSettingsModalComponent = React.useMemo(
    () => withOverride(uiOverrides?.translationSettingsModal, TranslationSettingsModal),
  ...
  const panelists = useRef<Participant[]>([]);
  const panelistsFocused = useRef<boolean>(false);
  const muteOthersMic = useRef<boolean>(false);
  const muteOthersCamera = useRef<boolean>(false);
  const permissionConfig = useRef<any>(null);
  ...
  const [isPanelistsModalVisible, setIsPanelistsModalVisible] =
    useState<boolean>(false); // True if the panelists modal is visible as boolean
  const [isPermissionsModalVisible, setIsPermissionsModalVisible] =
    useState<boolean>(false); // True if the permissions modal is visible as boolean
  ...
  const updatePermissionConfig = (value: any) => {
  ...
  const updateListenPreferences = (
    ...
    listenerTranslationOverrides.current = new Map(
      Array.from(listenPreferences.current.entries()).map(([speakerId, language]) => [
        speakerId,
        { speakerId, wantOriginal: false, preferredLanguage: language },
      ]),
    );
    setTranslationUiVersion((current) => current + 1);
  };
      ...
      if (existingTranslations) {
        for (const [existingLang, existingTranslationProducerId] of Object.entries(
          ...
          const shouldClose =
            wantOriginal
            || (!!normalizedLang
              && existingLang.toLowerCase() !== normalizedLang);
  ...
  const renderAttachedSidebarContent = ({
    defaultContent,
  }: {
    defaultContent: React.ReactNode;
    dimensions: { width: number; height: number };
  }) =>
    shouldUseSidebar && activeSidebarContent !== 'none' ? (
      attachedSidebarVisible ? (
  ...
  const customMenuButtons: CustomButton[] = [
    ...
    },
    {
      ...
      show: eventType.current === 'webinar' && islevel.current === '2',
    },
    {
      icon: 'user-shield',
      text: 'Permissions',
  ...
  const controlBroadcastButtons: ButtonTouch[] = [
...

    {
      // theme toggle button
      customComponent: (
        <FontAwesome5 name={isDarkMode ? 'sun' : 'moon'} size={24} color={neutralControlIconColor} solid />
      ),
    ...
    if (socketDefault.id) {
      if (!skipSockets) {
        ...
        socketDefault.on('addedAsPanelist', async (data: any) => {
          await addedAsPanelist({ data, showAlert });
        });

        socketDefault.on('removedFromPanelists', async (data: any) => {
          await removedFromPanelists({ data, showAlert });
        });

        socketDefault.on('controlMedia', async (data: any) => {
          await controlPanelistMedia({
            data,
            showAlert,
            clickAudio: () => {
              clickAudio({ parameters: { ...getAllParams(), ...mediaSFUFunctions() } });
            },
            clickVideo: () => {
              clickVideo({ parameters: { ...getAllParams(), ...mediaSFUFunctions() } });
            },
            audioAlreadyOn: audioAlreadyOn.current,
            videoAlreadyOn: videoAlreadyOn.current,
          });
      ...
      {!validated ? (
        ...
        <MainContainer style={containerStyle}>
          ...
          <MainAspect
            ...
            renderContent={renderAttachedSidebarContent}
          >
        ...
        <>
          ...
          <PanelistsModalComponent
            ...
            parameters={{
              ...
              updatePanelists,
              updatePanelistsFocused,
              getUpdatedAllParams: getAllParams as any,
            }}
          />
